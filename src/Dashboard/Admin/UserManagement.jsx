...
const UserManagement = () => {
    ...
    const handleAccountAction = async (userId, action) => {
        try {
            const response = await axios.put(`http://localhost:5000/users/${userId}/${action}`);
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {users.map((user) => (
                            <tr key={user._id}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">{user.account_status}</td>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    {user.account_status === 'pending' && (
                                        <button
                                            onClick={() => handleAccountAction(user._id, 'activate')}
                                    ...
                                    <button
                                        onClick={() => handleAccountAction(user._id, 'block')}
